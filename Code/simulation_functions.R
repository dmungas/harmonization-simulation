...
equateSim <- function(seed=NULL,grp_mean=c(0.5,-0.5),grp_sd=c(1,1),
                      ref_grp = 1,
                      n_samp1=500, n_samp2=500, n_rep_theta=1,n_itm=30,pars=mcal_pars,n_rep=100,
  ...
  link_itms <- itms1[itms1 %in% itms2]
  ...
  while (j < n_rep) {
    j <- j+1
    time <- Sys.time()
    theta1 <- data.frame(rnorm(n_samp1,grp_mean[1],grp_sd[1]))
    names(theta1) <- "theta1"
    theta1$group <- 1
    theta2 <- data.frame(rnorm(n_samp2,grp_mean[2],grp_sd[2]))
    names(theta2) <- "theta1"
    theta2$group <- 2
    ...
    for (i in 1:n_rep_theta){
      if (is.null(mod_res_obj)) {
        ds2 <- data.frame(simdata(a=disc,d=diff,N=n_samp1+n_samp2,Theta=as.matrix(theta1$theta1),
                                  itemtype=itemtype))
      } else {
        ds2 <- data.frame(simdata(model=mod_res_obj,N=n_samp1+n_samp2,
